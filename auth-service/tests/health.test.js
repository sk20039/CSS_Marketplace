...
(async () => {
  console.log('\n=== auth-service health endpoint tests ===');
  ...
  await test('GET /health/live returns 200 without touching the database', async () => {
    ...
    assertEqual(res.body.service, 'auth-service', 'service name must be auth-service');
  ...
  await test('health endpoints do not require Authorization header', async () => {
    // Confirm no 401 is returned without any token
  ...
  await test('GET /health/ready returns 503 when database is unavailable', async () => {
    ...
    failApp.use('/health', buildHealthRouter(brokenPool, 'auth-service'));
