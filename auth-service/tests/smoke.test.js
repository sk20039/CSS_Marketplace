// tests/smoke.test.js — auth-service deployment smoke check.
...
process.env.DATABASE_URL = process.env.DATABASE_URL_TEST || 'postgres://auth_user:x@127.0.0.1:5432/smoke';
...
(async () => {
  console.log('\n=== auth-service deployment smoke check ===');
  ...
  await check('GET /health/live returns 200 ok (no DB required)', async () => {
    ...
    if (body.service !== 'auth-service') throw new Error(`unexpected service name: ${body.service}`);
  });
