...
async function runFeeAmountTests() {
  ...
  for (const { priceCents, expectedFee, expectedPayout, label } of cases) {
    await testAsync(`Order at ${label}: fee=${expectedFee}¢, payout=${expectedPayout}¢`, async () => {
      ...
      assertEqual(order.shipping_cents,      0,          'shipping_cents must be 0 (free shipping for buyers)');
      assertEqual(order.amount_cents,        priceCents, 'amount_cents = item_price_cents (no shipping charge)');
...
async function runCancellationTests() {
  ...
  await testAsync('$100.00 cancellation: refund = $92.00, platform keeps $8.00', async () => {
    ...
    // Refund = amount_cents - platform_fee = 10000 - 800 = 9200 (no shipping in amount)
    assertEqual(event.payload.refundAmountCents,    9200, 'refundAmountCents must be item_price - fee');
  ...
  await testAsync('$20.00 cancellation: minimum fee $2.00 kept, refund = $18.00', async () => {
    ...
    assertEqual(res.status, 200, `cancel failed: ${JSON.stringify(res.body)}`);
    const event = res.body.events.find((e) => e.event_type === 'CANCELLED');
    // Refund = amount_cents - platform_fee = 2000 - 200 = 1800 (no shipping in amount)
    assertEqual(event.payload.refundAmountCents,    1800, 'refundAmountCents must be item_price - fee');
  ...
  await testAsync('$1.00 cancellation: fee covers full amount, refund = $0.00, no Stripe call', async () => {
    ...
    assertEqual(res.status, 200, `cancel failed: ${JSON.stringify(res.body)}`);
    const event = res.body.events.find((e) => e.event_type === 'CANCELLED');
    // Refund = amount_cents - platform_fee = 100 - 100 = 0 (fee capped at item price, no shipping)
    assertEqual(event.payload.refundAmountCents,    0, 'refundAmountCents must be 0 when fee equals full item price');
    assertEqual(event.payload.platformFeeKeptCents, 100, 'platformFeeKeptCents must be 100 (capped at item price)');
    // No Stripe refund call needed when refund amount is zero.
    assert(!res.body.stripe_refund_id, 'stripe_refund_id must not be set when refund amount is zero');
  });
}
...
async function runDisputeRefundTests() {
  ...
  await testAsync('$50.00 dispute refund: full amount returned to buyer', async () => {
    ...
    // amount_cents is item price only (no shipping charge to buyer)
    assertEqual(resolveRes.body.amount_cents, 5000, 'order amount_cents must be item price only (no shipping)');
  });
}
