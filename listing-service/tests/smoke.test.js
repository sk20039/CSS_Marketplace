// tests/smoke.test.js — listing-service deployment smoke check.
...
process.env.DATABASE_URL = process.env.DATABASE_URL_TEST || 'postgres://listing_user:x@127.0.0.1:5432/smoke';
process.env.JWT_SECRET = process.env.JWT_SECRET || 'smoke-check-placeholder-32chars!!';
process.env.INTERNAL_SERVICE_SECRET = process.env.INTERNAL_SERVICE_SECRET || 'smoke-internal-svc-secret-32chars!!';
...
(async () => {
  console.log('\n=== listing-service deployment smoke check ===');
  ...
  await check('GET /health/live returns 200 ok (no DB required)', async () => {
    ...
    if (body.service !== 'listing-service') throw new Error(`unexpected service name: ${body.service}`);
  });
