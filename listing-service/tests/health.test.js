...
// Health endpoint tests for listing-service.
...
process.env.DATABASE_URL =
  ...
  'postgres://listing_user:listing_pass@127.0.0.1:5432/listing_db_test';
...
(async () => {
  console.log('\n=== listing-service health endpoint tests ===');
  ...
  await test('GET /health/live returns 200 without touching the database', async () => {
    ...
    assertEqual(res.body.service, 'listing-service', 'service name must be listing-service');
  ...
  await test('GET /health/ready returns 503 when database is unavailable', async () => {
    ...
    failApp.use('/health', buildHealthRouter(brokenPool, 'listing-service'));
